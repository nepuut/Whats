   ...
   if (pairingCode && !XeonBotInc.authState.creds.registered) {
      ...
      if (!!phoneNumber) {
         ...
         if (!Object.keys(PHONENUMBER_MCC).some(v => phoneNumber.startsWith(v))) {
            console.log(chalk.bgBlack(chalk.redBright("Start with country code of your WhatsApp Number, Example : +923323001142")))
            process.exit(0)
         }
      } else {
         phoneNumber = await question(chalk.bgBlack(chalk.greenBright(`Please type your WhatsApp number 😍\nFor example: +923323001142 : `)))
         phoneNumber = phoneNumber.replace(/[^0-9]/g, '')
...
XeonBotInc.ev.on("connection.update",async  (s) => {
        ...
        if (connection == "open") {
			...
			await delay(1999)
            console.log(chalk.yellow(`\n\n                  ${chalk.bold.blue(`[ Ahmed Cheats ᵇᵘᵍ ᵇᵒᵗ ]`)}\n\n`))
            console.log(chalk.cyan(`< ================================================== >`))
	        console.log(chalk.magenta(`\n${themeemoji} YT CHANNEL: Ahmed Cheats`))
            console.log(chalk.magenta(`${themeemoji} GITHUB: AhmedCheats `))
            console.log(chalk.magenta(`${themeemoji} INSTAGRAM: @Ahmed_Op_iG `))
            console.log(chalk.magenta(`${themeemoji} WA NUMBER: 923323001142`))
            console.log(chalk.magenta(`${themeemoji} CREDIT: Ahmed Cheats Sir\n`))
        }
        if (
            connection === "close" &&
            lastDisconnect &&
